...
{
	vector<bool> visited(9, false);
	vector<bool> recstack(9, false);

	for(int i=0; i<9; i++)
	{
		if(isCyclic(tmpgraph, i, visited, recstack))
			return true;
	}
...
bool Graph::isCyclic(vector<vector<int> > tmpgraph, int vtx, vector<bool> &visited, vector<bool> &recstack)
{
	if(visited[vtx] == false)
	{
		...
		{
			if(visited[i] == false && tmpgraph[vtx][i] == 1 && isCyclic(tmpgraph, i, visited, recstack))
				return true;
			else if(tmpgraph[vtx][i] == 1 && recstack[i] == true)
				return true;
		}
	}
